...
public class Config {
    ...
    private static void setupItemDigitizerConfig(ForgeConfigSpec.Builder SERVER_BUILDER) {
        SERVER_BUILDER.comment("Item digitizer settings")
            .push(SUBCATEGORY_ITEM_DIGITIZER);

        ITEM_DIGITIZER_POWER_MAX = SERVER_BUILDER
            .comment("Maximum power for the item digitizer")
        ...
        ITEM_DIGITIZER_CHECK_COST = SERVER_BUILDER
            .comment("Power usage for the item digitizer to get information about an item stack")
            .defineInRange("checkCost", 10, 0, Integer.MAX_VALUE);
        ITEM_DIGITIZER_POWER_ENABLED = SERVER_BUILDER
            .comment("Toggles whether or not the item digitizer needs power to function")
